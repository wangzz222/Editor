...
function actionRevision (req, res, note) {
  ...
  if (actionId) {
    ...
    logger.info(`获取指定时间的修订版本: noteId=${note.id}, time=${time}`)
    
    if (!time.isValid()) {
      logger.warn(`获取修订版本失败: 无效的时间格式 ${actionId}`)
      return errorNotFound(req, res)
    }
    
    Revision.getPatchedNoteRevisionByTime(note, time, function (err, content) {
      if (err) {
        logger.error(`获取修订版本失败: ${err}`)
        return errorInternalError(req, res)
      }
      if (!content) {
        logger.warn(`获取修订版本失败: 未找到修订版本 noteId=${note.id}, time=${time}`)
        return errorNotFound(req, res)
      }
      
      logger.info(`成功获取修订版本: noteId=${note.id}, time=${time}`)
      res.set({
        'Access-Control-Allow-Origin': '*', // allow CORS as API
        'Access-Control-Allow-Headers': 'Range',
        'Access-Control-Expose-Headers': 'Cache-Control, Content-Encoding, Content-Range',
    ...
    logger.info(`获取修订版本列表: noteId=${note.id}`)
    Revision.getNoteRevisions(note, function (err, data) {
      if (err) {
        logger.error(`获取修订版本列表失败: ${err}`)
        return errorInternalError(req, res)
      }
      
      logger.info(`成功获取修订版本列表: noteId=${note.id}, count=${data.length}`)
      const result = {
        revision: data
      }
      res.set({
